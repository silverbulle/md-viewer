...
| 功能 | 说明 |
|------|------|
| **原生文件夹选择器** | 点击 📂 按钮弹 Windows 目录选择对话框 |
| **路径输入** | 同行输入框可直接输入路径，按 Enter 或点 Go 打开 |
| **记忆上次目录** | 自动记住上次打开的目录，下次启动直接恢复 |
| **可折叠目录树** | 文件夹展开/折叠，README 自动置顶 |
| **可拖拽调宽** | 拖动侧边栏右边缘调整宽度（180~520px），宽度跨会话记忆 |
| **一键收起** | 顶栏 ☰ 按钮平滑收起侧边栏，主内容占满；收起状态跨会话记忆 |
| **平滑过渡** | Files/Outline 切换淡入淡出，侧边栏收起/展开带宽度动画 |
...
| `--multi` | 允许多实例并行（跳过单实例检测） | false |

> **单实例**: 默认同一时刻只允许一个实例运行。重复启动 exe 会弹窗提示"已在端口 XXXX 运行"并打开已运行实例的浏览器窗口，而非新开一个。如需并行运行多个（如不同目录/端口），加 `--multi`。锁文件位于 `~/md-browser.lock`。
...
- **打开自适应**: 浮层打开时按图片原始尺寸与窗口自适应缩放（小图不放大，大图缩到可见）
- **Pandoc 属性修复**: `![alt](url){width="85%"}` 这种 `{...}` 写在图片标记**之外**的 Pandoc 语法，之前会作为纯文本残留在图片下方——现在通过渲染后 DOM 扫描，把紧跟图片的 `{width=.. height=..}` 吸收为 inline style 并清除文本
- **链接内图片不触发放大**: 图片若被 `<a>` 包裹（点击应跳转链接），则不绑定放大浮层
...
- *(注：`{width=..}` 属性在 v1.14 仅处理了写在 alt/title 内的写法；Pandoc 标准的标记外 `{...}` 写法在 v1.15 才完整修复)*

### v1.13 — UI 过渡平滑化 & 目录区合并
- **Files/Outline 淡入淡出**: tab 切换内容区先淡出再淡入（~120ms），不再瞬切
- **侧边栏收起动画**: 收起改为宽度塌缩到 0（带 0.2s 动画）而非直接消失，展开反向；拖拽条在收起态隐藏
- **拖拽跟手**: 拖拽调宽时禁用宽度过渡，侧边栏紧跟光标，松开恢复过渡
- **目录区合并**: 路径输入框与文件选择按钮合并为同一行（📂 + 输入框 + Go），不再独占两行；按钮简化为图标
- **过渡统一**: 侧边栏 transition 增加 opacity/border，手感一致

### v1.12 — 单实例检测
- **防重复启动**: 重复运行 exe 时弹窗提示"已在端口 XXXX 运行"，并直接打开已运行实例的浏览器窗口，不再新开第二个
- **端口指明**: 提示框明确告知当前实例运行的端口号和地址
- **僵尸锁自愈**: 锁文件指向的端口无人监听时（上次崩溃残留）自动清理并正常启动
- **`--multi` 逃生**: 需要并行多实例时加 `--multi` 跳过检测
- **清理机制**: 正常退出（atexit）时移除锁文件 `~/md-browser.lock`

### v1.11 — 侧边栏可调宽 & 可隐藏
- **拖拽调宽**: 侧边栏右边缘加拖拽条，按住拖动调整宽度（180~520px），实时生效
- **一键收起**: 顶栏 ☰ 按钮完全收起侧边栏，主内容区占满；再点恢复，平滑过渡
- **跨会话记忆**: 宽度和收起状态存入 localStorage，下次启动恢复
- **拖拽隔离**: 拖拽时禁用文本选中和指针干扰，体验顺滑
